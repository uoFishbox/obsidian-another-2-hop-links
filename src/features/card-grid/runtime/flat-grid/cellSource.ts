...
export interface FlatGridCellSource<T> {
 ...
 */
export function createFlatGridCellSource<T>(params: {
	header: boolean;
	items: readonly T[];
	visibleCount: number;
	...
	const headerCell: FlatGridLogicalCell<T> | null = hasHeader
		? {
				kind: "header",
				key: logicalCellKey(`flat:${encodedKeyPrefix}:header`),
			}
		: null;
	const loadMoreCell: FlatGridLogicalCell<T> | null = params.showLoadMore
		? {
				kind: "load-more",
	...
	const slotBindingRevision = params.slotBindingRevision ?? revision;
