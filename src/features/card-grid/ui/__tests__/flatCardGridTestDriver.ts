...
export interface RenderFlatCardGridOptions {
	items?: string[];
	showHeader?: boolean;
...
export interface FlatCardGridDriver {
...
function prepareFlatGridLayout(
	container: HTMLElement,
	options: {
		rootHeight: number;
		width: number;
...
export function renderFlatCardGrid(
	options: RenderFlatCardGridOptions = {},
): FlatCardGridDriver {
	const {
		items = createItems(6),
		showHeader = false,
	...
	const driver: FlatCardGridDriver = {
		container,
		scrollRoot,
		gridRoot,
...
export interface RenderFlatCardGridObjectOptions {
	items: HarnessItem[];
...
export interface FlatCardGridObjectDriver {
	container: HTMLElement;
	scrollRoot: HTMLElement;
	gridRoot: HTMLElement;
...
export function renderFlatCardGridObject(
	options: RenderFlatCardGridObjectOptions,
): FlatCardGridObjectDriver {
	const {
		items,
		itemsRevision,
		initialVisibleCount = items.length,
	...
	const driver: FlatCardGridObjectDriver = {
