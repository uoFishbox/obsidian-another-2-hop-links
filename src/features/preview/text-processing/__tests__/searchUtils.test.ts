...
describe("searchUtils", () => {
	it("treats space-separated query terms as separate matches", () => {
		...
		expect("Alpha gamma beta".match(pattern ?? /$^/g)).toEqual([
			"Alpha",
			"beta",
		]);
	});
	...
	it("finds the first matching query term", () => {
		expect(
			findCaseInsensitiveIndex("before beta then alpha", "alpha beta"),
		).toBe(7);
	});
});
...
describe("htmlVisibleTextContainsCaseInsensitive", () => {
	it("matches visible text across HTML tag boundaries", () => {
		...
		expect(
			htmlVisibleTextContainsCaseInsensitive(html, "hello world"),
		).toBe(true);
	});

	it("does not match text that only appears inside tags", () => {
		const html = '<div class="hello">visible</div>';
		expect(htmlVisibleTextContainsCaseInsensitive(html, "hello")).toBe(
			false,
		);
		expect(htmlVisibleTextContainsCaseInsensitive(html, "visible")).toBe(
			true,
		);
	});
	...
	it("keeps inline-code html-like text visible for query detection", () => {
		...
		expect(htmlVisibleTextContainsCaseInsensitive(html, "<tag>")).toBe(
			false,
		);
	});

	it("returns false for empty query", () => {
		expect(htmlVisibleTextContainsCaseInsensitive("<b>text</b>", "")).toBe(
			false,
		);
	});
	...
	it("returns false when needle is not in visible text", () => {
		expect(
			htmlVisibleTextContainsCaseInsensitive("<p>hello</p>", "world"),
		).toBe(false);
	});
	...
	it("does not decode HTML entities (consistent with stripHtmlTags)", () => {
		...
		const html = "<p>a &lt; b</p>";
		expect(htmlVisibleTextContainsCaseInsensitive(html, "a &lt; b")).toBe(
			true,
		);
		expect(htmlVisibleTextContainsCaseInsensitive(html, "a < b")).toBe(
			false,
