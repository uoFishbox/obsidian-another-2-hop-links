...
} from "features/two-hop/application/TwoHopApplicationStore.svelte";
...
describe("TwoHopApplicationStore (Runes)", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});
	...
	it("handleDataUpdate reloads and invalidates previews for a reload action", async () => {
		...
		const previousPreviewVersion =
			store.uiState.getPreviewRenderVersion("target.md");
		...
		expect(resolveTwoHopLinks).toHaveBeenCalledTimes(2);
		expect(store.uiState.getPreviewRenderVersion("target.md")).not.toBe(
