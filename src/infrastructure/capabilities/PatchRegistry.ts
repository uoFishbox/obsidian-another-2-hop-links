...
import type { CapabilityRisk } from "./ObsidianInternalFacade";
...
export type PatchSpec<T extends object, M extends MethodKeys<T> = MethodKeys<T>> = {
	...
	risk: CapabilityRisk;
	enabled: boolean;
	validate?: (original: unknown) => boolean;
...
export class PatchRegistry {
	...
	apply<T extends object, M extends MethodKeys<T>>(plugin: Plugin, spec: PatchSpec<T, M>): boolean {
		...
		if (spec.validate && !spec.validate(original)) {
			return false;
		}
		...
		this.active.add(spec.id);
		this.uninstallers.push(dispose);
		plugin.register(dispose);
		return true;
	}

	disposeAll(): void {
		for (const dispose of this.uninstallers.splice(0)) {
			dispose();
		}
	}
}