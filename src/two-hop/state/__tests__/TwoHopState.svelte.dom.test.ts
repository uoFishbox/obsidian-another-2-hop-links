...
import { TwoHopState, type DisplayDataBuilder } from "two-hop/state/TwoHopState.svelte";
...
describe("TwoHopState (Runes)", () => {
	...
	it("does not recompute displayData on setSectionExpandedLimit", async () => {
		...
		store.uiState.setSectionExpandedLimit("backlinks", 30);
		const expandedLimit = store.uiState.getSectionExpandedLimit("backlinks");
	...
	it("lazy sort resolver calls helper with current sortOption", () => {
		...
		store.uiState.setSortOption("modified-date");
	...
	it("setSortOption does not re-fire onSortChange for same value", () => {
		...
		store.uiState.setSortOption(DEFAULT_SETTINGS.lastUsedSortOption);
		expect(onSortChange).not.toHaveBeenCalled();

		store.uiState.setSortOption("modified-date");
		expect(onSortChange).toHaveBeenCalledTimes(1);
		expect(onSortChange).toHaveBeenCalledWith("modified-date");
		expect(store.uiState.sortOption).toBe("modified-date");
	});
	...
	it("handleDataUpdate only advances updateVersion for global list store without current file", async () => {
		...
		const { store } = createStore({ resolveTwoHopLinks });
		const previousVersion = store.uiState.updateVersion;

		await store.handleDataUpdate({
			affectedPaths: ["note.md"],
		});

		expect(store.uiState.updateVersion).toBe(previousVersion + 1);
	...
	it("handleDataUpdate reloads and invalidates previews for a reload action", async () => {
		...
		const previousPreviewVersion =
			store.uiState.previewState.getRenderVersion("target.md");
