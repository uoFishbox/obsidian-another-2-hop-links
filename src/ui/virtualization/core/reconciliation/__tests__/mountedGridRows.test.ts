...
function buildRows(params: {
	...
	const build = buildMountedGridRows<TestLogicalCell, TestMountedCell>({
		...
		bindCell: ({ cell, previous, rowIndex, columnIndex, renderSlotIndex }) => ({
			...bindCell(cell, previous, columnIndex, renderSlotIndex),
			rowIndex,
		}),
	});
...
describe("buildMountedGridRows", () => {
	...
	it("rebinds physical bindings when the logical row shell cannot be reused", () => {
		...
		const rebuilt = buildRows({
			rowModel: createRowModel([2]),
			allocator,
			previousRows: initial.build.rowSlices,
	...
	it("reuses occupied bindings and creates only columns transitioning from empty", () => {
		...
		const partial = buildRows({
			rowModel: createRowModel([2]),
			allocator,
			previousRows: full.build.rowSlices,
		});
		const restored = buildRows({
			rowModel: createRowModel([4]),
			allocator,
			previousRows: partial.build.rowSlices,
		});
	...
	it("orders rowsBySlot by physical slot independently of logical row order", () => {
		...
		expect(shifted.build.rowSlices.map((row) => row.slotIndex)).toEqual([
			2, 3, 0, 1,
		]);
		expect(shifted.build.rowsBySlot.map((row) => row.slotIndex)).toEqual([
			0, 1, 2, 3,
		]);
		expect(shifted.build.rowsBySlot.map((row) => row.rowIndex)).toEqual([
			4, 5, 2, 3,
		]);
	});
	...
	it("reuses unchanged resident row shells without resolving their row model", () => {
		...
		expect(reused.build.rowSlices[0]).toBe(initial.build.rowSlices[0]);
		expect(reused.build.rowSlices[1]).toBe(initial.build.rowSlices[1]);
