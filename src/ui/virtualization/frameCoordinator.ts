...
export function createVirtualFrameCoordinator(params: {
	...
	function cancelIdleDrain(): void {
		...
		const ownerWindow = resolveWindow();
		if (ownerWindow) {
			if (idleUsesCallback && typeof ownerWindow.cancelIdleCallback === "function") {
...
export function createCoordinatedScheduledTask(params: {
	...
	return {
		...
		isScheduled: () =>
			params.coordinator.isScheduled(params.lane, params.key),
	};
}
