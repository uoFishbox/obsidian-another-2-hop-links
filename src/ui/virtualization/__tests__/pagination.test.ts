...
describe("createSectionVisibleCountsController", () => {
	it("reconciles input without persisting automatic clamp or prune changes", () => {
		...
		const controller = createSectionVisibleCountsController<string, HarnessSection>(
			{
				applicationStore: {
		...
		const initial = controller.resolveForInput([createSection("section-a", 5)]);
		...
		const clamped = controller.resolveForInput([createSection("section-a", 3)]);
	...
	it("persists only explicit load-more changes", () => {
		...
		const setSectionExpandedLimit = vi.fn((sectionId: string, limit: number) => {
	...
	it("derives visible counts from current loaded count without storing small counts", () => {
		...
		const oneItem = controller.resolveForInput([createSection("section-a", 1)]);
		expect(oneItem.snapshot.visibleCounts).toEqual({ "section-a": 1 });
		expect(oneItem.snapshot.expandedLimits).toEqual({});

		const twoItems = controller.resolveForInput([createSection("section-a", 2)]);
	...
	it("keeps explicit expanded limits across temporary missing input sections", () => {
		...
		const restored = controller.resolveForInput([createSection("section-a", 6)]);
