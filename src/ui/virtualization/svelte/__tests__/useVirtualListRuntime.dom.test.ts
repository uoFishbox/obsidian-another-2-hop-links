...
import {
	useVirtualListRuntime,
	type UseVirtualListRuntimeOptions,
} from "../useVirtualListRuntime.svelte";
import UseVirtualListRuntimeHarness from "./UseVirtualListRuntimeHarness.svelte";
...
vi.mock("../../dom/virtualListDomObserver", async (importOriginal) => {
	...
	return {
		...actual,
		observeVirtualListViewport: (options: ObserveVirtualListViewportOptions) => {
...
const createRuntimeHarness = (
	...
	render(UseVirtualListRuntimeHarness, {
		initialize: () => {
			runtime = useVirtualListRuntime(options);
		},
	});
...
describe("useVirtualListRuntime", () => {
	beforeEach(() => {
		vi.useFakeTimers();
	...
	it("wires bootstrap suppression and stabilization through one lifecycle", () => {
		...
		const observerOptions =
			viewportObservationHarness.options as ObserveVirtualListViewportOptions;
