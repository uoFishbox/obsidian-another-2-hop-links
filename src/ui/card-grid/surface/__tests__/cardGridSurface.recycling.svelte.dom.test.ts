...
import VirtualSurfaceRecyclingHarness from "./CardGridSurfaceRecyclingHarness.svelte";
import { type MountedVirtualCell, type LogicalCellKey } from "ui/virtualization/public";
...
function createCells(keys: string[], slotOffset: number = 0): TestMountedCell[] {
	return keys.map((key, index) => ({
		...
		physicalCellSlot: slotOffset + index,
		rowIndex: 0,
		columnIndex: index,
		top: 0,
		left: index * 100,
...
function createRows(
	...
	const row = {
		...
		top: options.top ?? 0,
		physicalRowSlot: options.physicalRowSlot ?? 0,
		bindings: cells,
	};
	return [row];
}
